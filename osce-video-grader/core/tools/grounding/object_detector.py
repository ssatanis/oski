...
from core.prompts.gemini import OSCE_KEYFRAME_OBJECT_DETECTOR_PROMPT
...
from core.tools.schemas import ObjectDetectionOutput
...
def generate_keyframes_object_detection_evidence_with_gemini(
        gemini_client, 
    ...
    Returns:
        List[ObjectDetectionOutput]: List of descriptions for each keyframe.
    """
    try:
        gemini_image_processor = GeminiImageBatchProcessor(
            client=gemini_client,
            prompt_template=OSCE_KEYFRAME_OBJECT_DETECTOR_PROMPT,
...
def format_object_detector_output(
    retrieved_keyframes_results: List[SearchResult[VideoKeyframeMetadata]], 
    object_detection_results: List[ObjectDetectionOutput],
    minio_client: MinIOClient
):
    """
    Format the object detection results into a structured output.

    Args:
        results (List[ObjectDetectionOutput]): List of object detection results.
        retrieved_keyframes_results (List[SearchResult]): List of retrieved keyframes.

    Returns:
        List[dict]: Formatted output containing detected objects and their metadata.
    """
    object_detector_tool_output = []
    for kf_res, det in zip(retrieved_keyframes_results, object_detection_results):
        ts = kf_res.metadata.timestamp
        kf_id = kf_res.id
        image_url = minio_client.get_presigned_url(kf_res.metadata.minio_path)

        for obj in det["identified_objects"]:
            object_detector_tool_output.append({
                "name": obj["name"],
                "context": obj["context_or_location"],
                "timestamp": ts,
                "keyframe_id": kf_id,
    ...
    return object_detector_tool_output

class ObjectDetectorTool(Tool):
    TOOL_NAME = "object_detector"
    TOOL_CATEGORY = ToolCategory.VISUAL 
    TOOL_DESCRIPTION = (
        "This tool analyzes keyframes from an OSCE video to detect clinically relevant objects and their usage context. ",
        "The output of this tool is a list of detected objects, their context or location, and confidence scores. ",
        "The output will be used to provide evidence for the student's performance in the OSCE video based on a specific rubric question related to student's usage of clinical tools."
    )
    ...
    def run(
        ...
        try:
            ...
            object_detection_results = self.gemini_batch_image_processor.process_batch(
                pil_images=pil_images, 
                response_format="json"
            )
        
            formatted_output = format_object_detector_output(
...
if __name__ == "__main__":
    ...
    # Generate captions for keyframes using Gemini
    # keyframe_object_detection_results = generate_keyframes_object_detection_evidence_with_gemini(
    #     gemini_client=gemini_client, 
    #     pil_images=pil_images
    # )

    # print(keyframe_object_detection_results)

    # object_detector_tool_output = format_object_detector_output(
    #     retrieved_keyframes_results=retrieved_keyframes_results, 
    #     object_detection_results=keyframe_object_detection_results
    # )
    # print(object_detector_tool_output)
    ...
    object_detector_output = object_detector_tool.run(
        retrieved_keyframes_result=retrieved_keyframes_results
    )

    print(object_detector_output)
